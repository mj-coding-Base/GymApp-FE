...
export function SessionHistory({ trainerId, trainerName }: Readonly<SessionHistoryProps>) {
  ...
  const fetchSessions = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await getAllSessions2(1, 10, trainerId);
      console.log('API Response:', response); // Debug log
      
      if (response?.status === "SUCCESS") {
        // The sessions are in response.data.data based on your console log
    ...
    return (
      ...
      >
        <div className="flex flex-col">
          <span className="text-[#434745] text-[11.5px] font-medium">
            {date || 'N/A'}
          </span>
        </div>
        ...
        <div className="text-[#434745] text-[11.5px] font-medium">
            ...
            <div className="w-full">
              ...
              <div className="bg-[#F7F7F8] rounded-lg overflow-hidden w-full">
                <div className="grid grid-cols-3 bg-[#fac1be] py-3 px-4">
                  ...
                  <div className="text-[#454545] text-[11px]">Time</div>
